...
// ODE solvers
typedef runge_kutta_dopri5< Vector > stepper_type; //
typedef runge_kutta_cash_karp54< Vector > stepper_type1;

void ode_function_solve(Vector, Vector &, double);
void ode_function_feasibility(Vector, Vector &, double);
void ode_function_A_path_solve(Vector x, Vector &dxdt, double t);
Vector dxdt_func(Vector x, Vector u, Vector mu, Vector M, Vector J);
...
private:
	...
	Vector t_length; // Discretization length
	...
	Vector path_conjugate_points; // Stores the conjugate points for a set of a's
...
public:
	...
	void rod_solve(Vector); // Solve rod for one 'a'
	void rod_path_solve(Matrix); // Solve rod for a path of a's = A_path
	void LogSolutionData(Vector, const double, int); // Log solution data through the computation
	void LogSolutionDataG(Vector, const double, int); // Log solution data through the computation - for the solution with only \mu, M and J.
	void LogSolutionDataPath(Vector, const double, int); // Log solution data through the computation during solution of a path of a's

	// Is rod configuration feasible
	bool isRodFeasible(Vector a); // Check if conf. 'a' is feasible (stability and collision free) - discrete geometry version
	bool isRodFeasible_old(Vector a); // Check if conf. 'a' is feasible (stability and collision free) - full ode version
	bool Rod_Collision(); // Check if rod is in self-collision for one 'a' conf., checks all points on the rod
	int conjugate_point(Vector a); // Find the first conjugate point along the rod - discrete geometry version
	int conjugate_point_old(Vector a); // Find the first conjugate point along the rod - full ode version
	...
	void setC(Vector); // Set stiffness of rod
	...
	Vector get_conjugate_points_path();
	...
	void print_A_State(Vector) const;
	int min(int, int);
