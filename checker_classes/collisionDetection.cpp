...
#define CADLINK "/home/avishai/Downloads/omplapp/ompl/Workspace/helixplanner/simulator/"
...
int collisionDetection::collision_state(Matrix M, Vector q1, Vector q2)
// Returns 0 if no collision
{
	...
	collisionDetection::rod_EE.BeginModel();
	
    int EE_buffer = 7;

	for (int i=EE_buffer; i<(M.size()/3-EE_buffer*2); i++){  //for each point, not first/last 21 points
		PQP_REAL p1[3],p2[3],p3[3];
		p1[0] = M[3*i][0]; p1[1] = M[3*i][1]; p1[2] = M[3*i][2];  //P1
		p2[0] = M[3*i+1][0]; p2[1] = M[3*i+1][1]; p2[2] = M[3*i+1][2];  //P2
		p3[0] = M[3*i+2][0]; p3[1] = M[3*i+2][1]; p3[2] = M[3*i+2][2];  //P3
		collisionDetection::rod_EE.AddTri(p1,p2,p3,i);
	}

	collisionDetection::rod_EE.EndModel();
	...
	MRotX(M6rod,rot6+q1_6_init);  //link 6 rotate X
	MxM(R6rod,R5,M6rod); // Different rotation matrix for the rod
	...
	T2[0] =  0;  // Was originally T3 in Steve's code
	T2[1] =  0;
	T2[2] =  270;

	MxV(T3,R2,T2);
	VpV(T3,T2_t,T3); // Was originally T2 (not T2_t) in Steve's code
	...
	T2[0] =  72;//-13/2; - Updated measure 4/13/2017
	T2[1] =  0;
	T2[2] =  0;
	...
	T2[0] =  95;//86;//13/2+60; // Measurement of new printed EE - Updated measure 4/13/2017
	T2[1] =  0;
	T2[2] =  0;
	...
	T22[0] =  72;//-13/2; - Updated measure 4/13/2017
	T22[1] =  0;
	T22[2] =  0;
	...
	T22[0] =  95;//86;//13/2+60; // Measurement of new printed EE - Updated measure 4/13/2017
	T22[1] =  0.0;
	T22[2] =  0;
	...
	PQP_Tolerance(&res[50],R6rod,T7,&rod,R0,T0,&base,tolerance);
	PQP_Tolerance(&res[51],R6rod,T7,&rod,R1,T1,&link1,tolerance);
	PQP_Tolerance(&res[52],R6rod,T7,&rod,R2,T2_t,&link2,tolerance);
	PQP_Tolerance(&res[53],R6rod,T7,&rod,R3,T3,&link3,tolerance);
	PQP_Tolerance(&res[54],R6rod,T7,&rod,R4,T4,&link4,tolerance);
	PQP_Tolerance(&res[55],R6rod,T7,&rod,R5,T5,&link5,tolerance);
	PQP_Tolerance(&res[56],R6rod,T7,&rod,R6,T6,&link6,tolerance);
	PQP_Tolerance(&res[57],R6rod,T7,&rod,R02,T02,&base2,tolerance);
	PQP_Tolerance(&res[58],R6rod,T7,&rod,R12,T12,&link12,tolerance);
	PQP_Tolerance(&res[59],R6rod,T7,&rod,R22,T2_t2,&link22,tolerance);
	PQP_Tolerance(&res[60],R6rod,T7,&rod,R32,T32,&link32,tolerance);
	PQP_Tolerance(&res[61],R6rod,T7,&rod,R42,T42,&link42,tolerance);
	PQP_Tolerance(&res[62],R6rod,T7,&rod,R52,T52,&link52,tolerance);
	PQP_Tolerance(&res[63],R6rod,T7,&rod,R62,T62,&link62,tolerance);
	
	// rod collisions with EE's with rod not counting buffer from parent EE
    PQP_Tolerance(&res[64],R6rod,T7,&rod_EE,R6,T7,&EE,tolerance);
	PQP_Tolerance(&res[65],R6rod,T7,&rod_EE,R62,T72,&EE2,tolerance);
...
collisionDetection::collisionDetection(double X, double Y, double Z, double Rot, double q1_6, double q2_6){
